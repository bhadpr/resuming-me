-- Seed ~30 days of demo metrics for Insights: Weight, Sleep, Mood.
-- Target: bhadpr@gmail.com
-- Replaces existing entries for those metrics only.
...
begin
  ...
  -- Weight (lbs) — gradual drift down with weekly noise
  ...
  -- Sleep (hrs) — lower on weekends, occasional bad nights
  select id into v_metric_id
  from public.metrics
  ...
  -- Mood (1–5) — dips correlate with poor sleep days
  select id into v_metric_id
  from public.metrics
